...
$Full | ForEach-Object -Process {
	...
	$UiData.Header | Add-Member -Name $ID -MemberType NoteProperty -Value $ItData.Header.$ID -Force
	$UiData.Description | Add-Member -Name $ID -MemberType NoteProperty -Value $ItData.Description.$ID -Force
	...
	if ($UiData.ChildElements)
	{
		$UiData.ChildElements | ForEach-Object -Process {
			...
			$UiChild.ChildHeader | Add-Member -Name $ID -MemberType NoteProperty -Value $ItChild.ChildHeader.$ID -Force
			$UiChild.ChildDescription | Add-Member -Name $ID -MemberType NoteProperty -Value $ItChild.ChildDescription.$ID -Force
		}
	}
}
